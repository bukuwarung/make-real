...
export class WebEditorFrameShapeUtil extends BaseBoxShapeUtil<WebEditorFrameShape> {
	...
	override canEdit = (shape: WebEditorFrameShape) =>
		shape.props.parts.length > 0 || shape.props.url.length > 0

	override isAspectRatioLocked = (_shape: WebEditorFrameShape) => false

	override canResize = (_shape: WebEditorFrameShape) => true

	override component(shape: WebEditorFrameShape) {
				...
				{isLoading ? (
					...
					<>
						...
						{isOnlySelected && (
							...
							>
								<Dropdown boxShadow={boxShadow} html={shape.props.url} uploadUrl={uploadUrl}>
									<button
	...
	override toSvg(shape: WebEditorFrameShape, _ctx: SvgExportContext) {
	...
	indicator(shape: WebEditorFrameShape) {
...
function PreviewImage({ shape, href }: { shape: WebEditorFrameShape; href: string }) {
